...
int main(int argc, char *const argv[])
{
    ...
    if (prg_prefix[0] == '"' && prg_prefix[prg_prefix.size() - 1] != '"')
    {
        std::string cdCommand = "cd " + prg_prefix + '"';
        std::system(cdCommand.c_str());
    }
    else
    {
        std::string cdCommand = "cd " + '"' + prg_prefix + '"';
    ...
    if(!song_tbl_ptr)
    {
        ...
        sappy_detector_cmd = "sappy_detector \"" + inGBA_path + '"';
#ifdef QT_DEBUG
    ...
    for(i = 0; i < song_list.size(); i++)
    {
        if(song_list[i] != song_tbl_end_ptr)
        {
            ...
            std::string seq_rip_cmd = "song_ripper \"" + inGBA_path + '"' + " \"" + outPath + "\\" + name;
    ...
    if(sb)
    {
        ...
        for(bank_t j=sound_bank_list.begin(); j!=sound_bank_list.end(); ++j)
        {
            ...
            std::string sf_rip_args = "sound_font_ripper \"" + inGBA_path + '"' + " \"" + outPath + "\\" + name + '/';
    ...
    else
    {
        ...
        std::string sf_rip_args = "sound_font_ripper \"" + inGBA_path + '"' + " \"" + outPath + "\\" + name + '/' + name + ".sf2\"";
        if(sample_rate) sf_rip_args += " -s" + std::to_string(sample_rate);
        if(main_volume) sf_rip_args += " -mv" + std::to_string(main_volume);
